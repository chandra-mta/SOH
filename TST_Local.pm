...
Time::TST_Local - efficiently compute time from local and GMT time
...
sub cheat {
    ...
    $year -= 1900
    	if $year > 1900;
    $month = $_[4];
    croak "Month '$month' out of range 0..12"	if $month > 12 || $month < 0;
    croak "Day '$_[3]' out of range 1..31"	if $_[3] > 31 || $_[3] < 1;
    croak "Hour '$_[2]' out of range 0..24"	if $_[2] > 24 || $_[2] < 0;
    croak "Minute '$_[1]' out of range 0..60"	if $_[1] > 60 || $_[1] < 0;
    croak "Second '$_[0]' out of range 0..60"	if $_[0] > 60 || $_[0] < 0;
    $guess = $^T;
    @g = gmtime($guess);
